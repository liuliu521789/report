...
import { tryNotifyContractReviewerOnSubmit } from '../../lib/wecomNotify.js';
import {
...
router.get('/contracts/:id/versions/:v1/:v2/diff', async (req, res, next) => {
  try {
    ...
    if (diff?.error === 'VERSIONS_NOT_FOUND') {
      return res.status(404).json({ error: 'VERSIONS_NOT_FOUND', message: '找不到可对比的两个版本（首版无法与上一版对比）' });
    }
    res.json(diff);
  } catch (e) {
    next(e);
